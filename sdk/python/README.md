...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `bunchc` with the `ORG` of your provider.
   - replace all instances of the `github.com/bunchc/pulumi-tf-provider-konnect` repository with the `REPOSITORY` location
